...
>(
  (
    { label, error, options, placeholder, onChange, className, name, ...props },
    ref,
  ) => (
    <div className={cn(`${label ? "space-y-2" : ""}`, className)}>
      ...
      >
        <SelectPrimitive.Trigger
          ref={ref}
          className={cn(
            "flex h-9 w-full items-center justify-between rounded-md border border-border bg-card px-3 py-2 text-sm shadow-sm transition-colors focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
